...
class Loss:
    ...
    def calculate(self, output, y, *, include_regularization=False):
        ...
        # Add accumlated sum of losses and sample count
    ...
    def calculate_accumulated(self, *, include_regularization=False):
        ...
        # Return data and regularlization loss
        return data_loss, self.regularization_loss()
...
# Exponentional
def exp_data(samples=1000):
    X = np.arange(samples).reshape(-1, 1) / samples
    y = np.exp(X).reshape(-1, 1)

    return X, y

# Square Root
def sqrt_data(samples=1000):
    X = np.arange(samples).reshape(-1, 1) / samples
    y = np.sqrt(X).reshape(-1, 1)
...
class Model:
    ...
    def train(self, X, y, *, epochs=1, batch_size=None, print_every=1,
        ...
        if batch_size is not None:
            ...
            # Add '1' to include if this not full batch
    ...
    def get_parameters(self):
        ...
        # Iterate training layers and get their parameters
        for layer in self.trainable_layers:
    ...
    def save(self,path):
        ...
        # Reset accumlated values in loss and accuracy objects
        model.loss.new_pass()
        ...
        # Open a file with the binary-write mode and save the model
        with open(path, 'wb') as f:
            pickle.dump(model,f)
...
### Flatten data (since there is no flatten layer in thi model)
...
# Train
model.train(x_train, y_train,validation_data=(x_test, y_test), epochs=5, batch_size=128, print_every=100)
